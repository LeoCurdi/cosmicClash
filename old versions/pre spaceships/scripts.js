...
var projectiles = [], enemyArray = [], particles = [], ammoBlobs = [], bombArray = [];
...
var ammoRadius = 13*scale, ammoSpawnFreq, ammoSpawnFreq1 = 0.0016, ammoStart = 50, ammoIncrements = 25;
...
function startGame() {
    ...
    particles = [], enemyArray = [], projectiles = [], ammoBlobs = [], bombArray = [];
...
function shootProjectile(event) {
    ...
    if (ammoTrack > 0) { // keep tack of ammo. only shoot if player has ammo
        ...
        projectiles.push({ x: player.x, y: player.y, size: projRadius, color: '#fff', velocity });
        ammoTrack--;
        ammoDisplay.innerHTML = ammoTrack;
    }
...
// new animation frame stuff
    ...
    console.log("Time: ", time, ", frameLength: ", timeElapsed);
...
// update the ramp-up aspects of the early game
    ...
    if (ammoTrack === 0 && ammoBlobs.length < 4) { // decrease the odds of getting unlucky with ammo spawns
        ammoSpawnFreq = 0.004;
    }
...
// update stuff
    ...
    for (var i = 0; i < particles.length; i++) {
        ...
        particles[i].size -= 0.04; // increase x pos by whatever the specific item's x speed is
    ...
    for (var i = 0; i < ammoBlobs.length; i++) {    
        if (ammoBlobs[i].pulse === 1) {
            if (ammoBlobs[i].radius < ammoRadius + 1.5) {
                ammoBlobs[i].radius += .5;
            }
            else {
                ammoBlobs[i].radius -= .15;
                ammoBlobs[i].pulse = -1;
            }
        }
        else if (ammoBlobs[i].pulse === -1) {
            if (ammoBlobs[i].radius > ammoRadius - 1.5) {
                ammoBlobs[i].radius -= .15;
            }
            else {
    ...
    for (var i = 0; i < projectiles.length; i++) {
        ...
        else if (projectiles[i].y < 0 || projectiles[i].y > canvas.height) {
            projectiles.splice(i, 1);
            i--;
        }
    }
...
// end game
    ...
    for (var i = 0; i < enemyArray.length; i++) {
        ...
        if (distance < enemyArray[i].size && enemyArray[i].isDead === 0) { // if center of player is within the enemy's circumference, and the enemy wasnt just killed by a bomb, game over
            console.log("enemies: ", enemyArray.length, " bombs: ", bombArray.length, " projectiles: ", projectiles.length, " particles: ", particles.length, " ammo: ", ammoBlobs.length);
...
// check for projectile collision with enemies. This is fucked because it checks each enemy against each projectile
    ...
    for (var i = 0; i < ammoBlobs.length; i++) {
        var dx = player.x - ammoBlobs[i].x; // center of player - center of ammo
        var dy = player.y - ammoBlobs[i].y; // same thing for y
        ...
        if (distance < player.radius + ammoRadius) { // if center of ammo blob is within the players circumference, add ammo
            ...
            ammoDisplay.innerHTML = ammoTrack;
            ammoBlobs.splice(i, 1); // this removes the food item (i) that collided with the player from the array
...
// draw stuff (in order of whats behind what in the event of overlap)
    ...
    for (var i = 0; i < ammoBlobs.length; i++) {
        context.beginPath();
        context.arc(ammoBlobs[i].x, ammoBlobs[i].y, ammoBlobs[i].radius, 0, 2 * Math.PI);
        context.fillStyle = 'yellow';
        context.fill();
        context.beginPath();
        context.arc(ammoBlobs[i].x, ammoBlobs[i].y, ammoBlobs[i].radius * innerDotScale, 0, 2 * Math.PI);
...
// generate stuff
    ...
    if (Math.random() < enemySpawnFreq) { // the number controls the frequency that food is generated. random gives u a random decimal from 0-1. so if num < .05, then we will generate a new food every 20 iterations (which is probably like 1/3 of a second)
        ...
        var isDead = 0;
        enemyArray.push({ x: x, y: y, size: size, dx: dx, dy: dy, color: color, isDead: isDead});
    }
    ...
    if (Math.random() < ammoSpawnFreq) {
        ...
        var pulse = 1;
        ammoBlobs.push({ x: x, y: y, radius: radius, pulse: pulse});
    }
